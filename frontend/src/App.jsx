...
export default function App() {
    ...
    const fetchAll = async () => {
      try {
        const [h, i, g] = await Promise.all([
          ...
          getJson(`${API_URL}/info`),
          getJson(`${API_URL}/gitops-status`),
        ]);
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ color: "#a6e3a1", fontSize: 16, marginBottom: 12 }}>
          ✅ Health
        </h2>
        {health ? (
          <>
            <div>
              Status: <strong style={{ color: "#a6e3a1" }}>{health.status}</strong>
            </div>
      ...
      >
        <h2 style={{ color: "#89b4fa", fontSize: 16, marginBottom: 12 }}>
          📦 App Info
        </h2>
        {info ? (
          <div
      ...
      >
        <h2 style={{ color: "#f9e2af", fontSize: 16, marginBottom: 12 }}>
          🔄 Pipeline Stages
        </h2>
        {gitops ? (
          <>
            ...
            >
              ...
              <strong style={{ color: "#a6e3a1" }}>
                {gitops?.argocd?.sync_status ?? "N/A"}
              </strong>
              {" · "}
              Health:{" "}
              <strong style={{ color: "#a6e3a1" }}>
                {gitops?.argocd?.health_status ?? "N/A"}
              </strong>
              {" · "}
              Repo:{" "}
              <strong style={{ color: "#cba6f7" }}>
                {gitops?.argocd?.repo ?? "N/A"}
              </strong>
            </div>
          </>
        ) : (
